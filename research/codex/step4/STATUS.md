...
Dong explicitly requested generated/degraded examples and qualitative validation at transcript times 14:04–15:57, then returned to validation and generative alternatives at 22:34–23:04. The previously sent Step-0 PDF contained verified examples. Advisor-only DOCX/PPTX companions with three clean current Step-4 rows were generated by allocated Slurm CPU job `4399697`; structural/hash checks pass. A fourth authentic row was excluded from the advisor display because its raw BRSET file contains a prominent horizontal acquisition seam; allocated audit `4399692` proved the seam predates transformation and report assembly. The companions remain private and Git-ignored because they embed licensed training images. Refresh them with the verified seed-0 result before the next advisor update.

## Seed-0 screen completed — September 15, 2026

Array `4397838` completed all A1/A2/A3 tasks successfully at 5,775 updates and 369,600 training draws per arm. All three task stderr logs are empty, contracts use the full joint B1 pool and common seed-0 initialization, and no Step-4 assessment/test artifact exists.

Original summary job `4397839` failed before comparison because it incorrectly applied the Step-4 no-test guard to the historical B1 directory, which legitimately contains its earlier Step-2 assessment. The verifier was corrected to read only B1 validation evidence while continuing to require A1/A2/A3 to be test-free. Allocated rerun `4401233` completed exit `0:0`; selection-summary and checkpoint hashes passed.

On the common 725-image mBRSET validation set, B1 DR/ME F1 was 0.8703/0.8489. A1 was 0.8772/0.8462, A2 was 0.8739/0.8485 and A3 was 0.8765/0.8485. DR F1 deltas were +0.0069/+0.0036/+0.0062; ME deltas were -0.0028/-0.0004/-0.0004. A3 DR AUROC improved by +0.0038; other AUROC changes were small. No arm met the frozen requirement that at least one F1 delta reach +0.01, so none passes automatic replication.

This is a negative seed-0 mechanism screen. Global appearance matching did not yield a large diagnostic gain. Preserve all arms as evidence, perform no A-arm test assessment and do not automatically launch seeds 1/2. Durable review: `SEED0_RESULTS_REVIEW.md`; aggregate evidence: `seed0_validation_summary.json`. Public living reports and private advisor companions were rebuilt by jobs `4401241` and `4401243` and passed their checks.
