...
def validate_manifest(path,expected_hash):
    ...
    expected = d.original_split.map({'train':'fit','val':'selection','test':'assessment'})
    if expected.isna().any() or not expected.eq(d.role).all():raise ValueError('Original split/role mismatch')
