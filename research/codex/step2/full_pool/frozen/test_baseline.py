...
class CoreChecks(unittest.TestCase):
    ...
    def test_manifest_role_leak_is_rejected(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            d=pd.DataFrame([{'domain':'mBRSET','file':str(i),'patient_key':'m:1','role':role,'original_split':{'fit':'train','selection':'val'}[role],LABELS[0]:0,LABELS[1]:1} for i,role in enumerate(['fit','selection'])]);d.to_csv(p,index=False)
            with self.assertRaisesRegex(ValueError,'Patient crosses'):validate_manifest(p,sha(p))
    def test_original_test_cannot_be_fit(self):
        with tempfile.TemporaryDirectory() as tmp:
            p=Path(tmp)/'m.csv'
            d=pd.DataFrame([{'domain':'mBRSET','file':'x','patient_key':'m:1','role':'fit','original_split':'test',LABELS[0]:0,LABELS[1]:1}])
            d.to_csv(p,index=False)
            with self.assertRaisesRegex(ValueError,'split/role'):validate_manifest(p,sha(p))
            d['role']='assessment';d.to_csv(p,index=False)
            self.assertEqual(len(validate_manifest(p,sha(p))),1)
...
if __name__=='__main__':
    ...
    out=Path('/home/users/sthummala2/brset-convnextv2/research/codex/step2/full_pool/cpu_checks.json')
